...
class MMDetWrapper(nn.Module):
    ...
    def __init__(self,
        ...
        if num_input_images > 1:
            self.model.backbone.conv1 = nn.Conv2d(num_input_images * 3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        if checkpoint_path is not None:
            loaded = torch.load(checkpoint_path, map_location=torch.device('cpu'))
            if num_input_images > 1:
                loaded['backbone.conv1.weight'] = torch.cat([loaded['backbone.conv1.weight']] * num_input_images, 1) / num_input_images
            mis,uxp = self.model.load_state_dict(loaded)  # otherwise all the processes will put the loaded weight on rank 0 and may lead to CUDA OOM
            print(f"loaded from {checkpoint_path},{mis},{uxp}")
        else:
            print("no pretrained backbone")
