...
#@hydra.main(config_name="./datasets/semantic_kitti/conf.yaml")
def main(config_path):
    ...
    trian_loader = data_module.train_dataloader()#.train_dataloader() #trainval_dataloader()#
    val_loader = data_module.val_dataloader() ##.test_dataloader()#
    ...
    if config.resume:
        ...
        model.load_save_dicts(checkpoint)
    ...
    for epoch in range(config.start_epoch, config.epochs+2):
        ...
        if (epoch + 1) % config.val_rate ==  0 and epoch>start_val:#:#or Trueor True 
            ...
            test_stats = evaluate(model,  val_loader,path = new_vis_dir,epoch = epoch)#val_loadertrian_loaderor True
            log_stats = {**{f'test_{k}': v for k, v in test_stats.items()},
                        'epoch': epoch,
                        'n_parameters': n_parameters,}
...
if __name__ == "__main__":
    main("models/confs/conf_psuedo.yaml")
